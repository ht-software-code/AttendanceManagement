...
namespace AttendanceManagement.Server.Controllers
{
    /// <summary>
    /// アカウントコントローラー
    /// URL:https://localhost:XXXXX/account/【Route name】
    /// </summary>
    [ApiController]
    [Route("[controller]")]
    public class AccountController : ApiControllerBase
    {
        ...
        /// <summary>
        /// JWT認証の有効ユーザがあれば応答する
        /// URL:https://localhost:XXXXX/account/usercurrent
        /// </summary>
        ...
        public Task<ActionResult<ServiceResult<UserResult?>>> Login([FromBody] LoginParam model)
        {
            return this.ExecuteLogic<ServiceResult<UserResult?>>(async () =>
            {
                ...
                {
                    Logger.Error($"{nameof(this.UserManager)} is null.");
                    return Unauthorized(ServiceResult<UserResult?>.CreateError(ApplicationErrorCodes.UnexpectedError));
                }

                Users? identityUser = await this.UserManager.FindByNameAsync(model.UserCode);
                if (identityUser == null)
                {
                    return Unauthorized(ServiceResult<UserResult?>.CreateError(ApplicationErrorCodes.LoginError));
                }
                ...
                if (!passwordValid)
                {
                    ...
                    return Unauthorized(ServiceResult<UserResult?>.CreateError(ApplicationErrorCodes.NotActiveUserError));
                }
        ...
        public Task<ActionResult<ServiceResult<bool?>>> CreateUser(UserCreateParam account)
        {
            return this.ExecuteLogic<ServiceResult<bool?>>(async () =>
            {
                ...
                if (!result.Succeeded)
                {
                    ...
                    return StatusCode(500, ServiceResult<bool?>.CreateError(ApplicationErrorCodes.AccounCreatFailedError));
                }
